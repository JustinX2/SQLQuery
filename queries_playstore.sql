...
-- Find ID and app name for all apps last updated on Aug 01, 2018
SELECT id, app_name FROM analytics WHERE last_updated = '2018-08-01';
-- Count the number of apps in each category
SELECT category, COUNT(*) FROM analytics GROUP BY category;
-- Find the top 5 most-reviewed apps
SELECT app_name, reviews FROM analytics ORDER BY reviews DESC LIMIT 5;
-- Find the app that has the most reviews with a rating >=4.8
SELECT app_name, MAX(reviews) FROM analytics WHERE rating >= 4.8 GROUP BY MAX(reviews) DESC LIMIT 1;
-- Find the avg rating for each category ordered by the highest rated to lowest rated
...
SELECT app_name, reviews FROM analytics WHERE price BETWEEN 0.10 AND 10 ORDER BY reviews DESC LIMIT 10;
...
SELECT app_name, reviews, min_installs FROM analytics WHERE min_installs >= 100000 ORDER BY min_installs / reviews DESC LIMIT 1;
